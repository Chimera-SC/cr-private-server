...
public class PacketDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		if (hasHeader) {
			if (in.readableBytes() < header.payload.length) return;
    ...
    private ClientMessage processPacket() {
        ...
        if (message == null) {
            String name = null;
            if (Messages.messagesMap.containsKey(header.id)) name = Messages.messagesMap.get(header.id);
        ...
        try {
            ...
            // Messages must be decoded fully, because they can contain useful information.
            if (!dataStream.eof()) {
                logger.warn(
					"Message %s is not decoded fully.\n\tDecoded part:\n%s\n\tOther part:\n%s",
					message.getClass().getSimpleName(),
					Hex.dump(header.decrypted, 0, dataStream.getOffset()),
					Hex.dump(
						header.decrypted,
						dataStream.getOffset(),
						header.decrypted.length - dataStream.getOffset()
					)
                );
            }
        } catch (Exception e) {
